...
```js
const runner_instance = new runner({ actions, validator });

await runner_instance.run({
  steps: [
    {
      step_id: "load",
      action: "LOAD_USER",
...
```powershell
node --input-type=module -e "import assert from 'node:assert/strict'; import {runner as runner_class} from './code/plugins/code_shared_runner_v3_0_0_draft.js'; import {validator as validator_class} from './code/plugins/code_shared_validator_v3_0_0_draft.js'; const calls=[]; const validator_instance=new validator_class(); const runner_instance=new runner_class({validator:validator_instance, actions:{execute_action:async (action, inputs)=>{calls.push({action, inputs}); return action==='CHOOSE'?'yes':inputs;}}, limits:{actions:10, depth:3}}); const inline=await runner_instance.run({steps:[{step_id:'s1', action:'ECHO', conditions:[{left:'{{input.ok}}', operator:'===', right:true}], inputs:{value:'{{input.value}}'}}]}, {ok:true,value:42}); assert.deepEqual(inline.s1,{value:42}); runner_instance.register_plan('jump',{steps:[{step_id:'choose', action:'CHOOSE', next_map:{yes:'done'}},{step_id:'skip', action:'SKIP'},{step_id:'done', action:'DONE', inputs:{ok:true}}]}); const jumped=await runner_instance.run('jump'); assert.deepEqual(jumped.done,{ok:true}); assert.throws(()=>runner_instance.topological_sort([{task_id:'a', dependencies:['b']},{task_id:'b', dependencies:['a']}]),/cycle/); assert.throws(()=>runner_instance.topological_sort([{task_id:'a', dependencies:['z']}]),/dependency/); assert.throws(()=>runner_instance.topological_sort([{task_id:'a'},{task_id:'a'}]),/Duplicate/); assert.throws(()=>runner_instance.topological_sort([{dependencies:[]}]),/missing task_id/); const order=runner_instance.topological_sort([{task_id:'b',dependencies:['a']},{task_id:'a'}]).map(t=>t.task_id); assert.deepEqual(order,['a','b']); const dag=await runner_instance.run({tasks:[{task_id:'first', action:'ECHO', inputs:{v:{source_type:'config', value:1}}},{task_id:'second', action:'ECHO', dependencies:['first'], inputs:{prev:{source_type:'task_output', task_id:'first'}}}]}); assert.deepEqual(dag.second,{prev:{v:1}}); console.log('runner checks passed');"
```
