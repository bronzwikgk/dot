...
test("product_surface validates built in datasets", function () {
  const surface = new product_surface();
  const result = surface.validate_datasets();
  assert.equal(result.ok, true);
});

test("product_surface registers all required templates", async function () {
...
test("product_surface instantiates application builder template", async function () {
  const templates = await read_templates();
  const surface = new product_surface({ templates }, {
  ...
  assert.ok(result.entities.some(function (entity) { return entity.type === "route"; }));
  assert.ok(result.entities.some(function (entity) { return entity.type === "view"; }));
});

test("product_surface rejects invalid route path", function () {
  const surface = new product_surface();
...
test("product_surface projects same entity into multiple layouts", function () {
  const surface = new product_surface();
  ...
  assert.equal(table_projection.projection.layout, "table_view");
});

test("product_surface creates command editor preview and e2e contracts", function () {
  const surface = new product_surface();
