...
async function read_surface_file(file_name) {
  return readFile(join(process.cwd(), "html", "product_surface", file_name), "utf8");
}

test("visible product surface has required static files and hooks", async function () {
...
test("visible product surface keeps class based browser controller", async function () {
  ...
  assert.equal(browser_source.includes("function "), false);
  assert.equal(browser_source.includes("=>"), false);
});

test("visible product surface css has responsive layout guards", async function () {
