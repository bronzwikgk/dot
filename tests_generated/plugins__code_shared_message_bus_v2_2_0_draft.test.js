// AUTOGENERATED by pipeline system_validate_and_test_code_shared_v2 for code_shared_message_bus_v2_2_0_draft
// regenerate instead of editing
const test = require("node:test");
const assert = require("node:assert");
const fs = require("node:fs");
const path = require("node:path");
const mod = require("E:\\root_0dot1_wip\\shared_v2\\code\\plugins\\code_shared_message_bus_v2_2_0_draft.js");
const SNAPSHOT_DIR = __dirname;
const SNAPSHOT_PATH = path.join(SNAPSHOT_DIR, "snapshots/plugins__code_shared_message_bus_v2_2_0_draft.snap.json");
...
test("create_message_bus [determinism] argset_0", () => {
  const __args = [];
  const first = __attempt("create_message_bus", __args);
  const second = __attempt("create_message_bus", __args);
  assert.deepStrictEqual(second, first, "nondeterministic behaviour");
});

test("create_message_bus [edge_safety] edge_0", () => {
  const __args = [];
  try {
    __call("create_message_bus", __args);
  } catch (error) {
